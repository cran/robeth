...
C.......................................................................
C
      DIMENSION T(MDX,NU),SZ(MDZ,NU),DL(NOBS),EL(NOBS),WGT(NOBS),
     *      SS(5*NCOV),IWORK(40)
...
C-----------------------------------------------------------------------
C
      SUBROUTINE BIGGBI(X,DNI,DMIN,DMAX,Y)
C.......................................................................
C
C   COPYRIGHT 1992 Alfio Marazzi
C
...
C   
     ...
     3       TOLA.GT.0..AND.B.GT.SQRT(FLOAT(NP)).AND.C.GT.0.
      ...
      ISD=IST+NCOV
      ISU=ISD+NP
      CALL GITAC2(X,Y,NI,OI,MDX,MDT,N,NP,NCOV,ICASE,MAXTT,MAXTA,
     +TOLT,TOLA,B,C,NITT,NITA,SIGMA,A,THETA,CI,DIST,RW1,RW1(IWG),
     +RW1(ISC),RW1(ISF),RW1(ISG),RW1(ISH),RW1(ISY),RW1(ISW),
     +RW2,IW1,DW1,DW1(IST),DW1(ISD),DW1(ISU))
      RETURN
      END
C 
C-----------------------------------------------------------------------
C
      SUBROUTINE GITAC2(X,Y,NI,OI,MDX,MDT,N,NP,NCOV,ICASE,MAXTT,
...
C  INITIAL VALUES FOR THE LOGISTIC REGRESSION
C
      DIMENSION X(MDX,NP),Y(N),COV(NCOV),CI(N),THETA(MDT),WGT(N),DIST(N)
     + ,OI(N),SC(NCOV),SF(NCOV),SG(NCOV),SH(NCOV),SX(MDX,NP),SY(N),SW(N)
      DOUBLE PRECISION A(NCOV),UCV,SA(NCOV),SD(NP),ST(NCOV),SU(N),Z
      INTEGER NI(N),SP(NP)
      LOGICAL NPRCHK
     ...
     2       .AND.MAXTT.GT.0.AND.MAXTA.GT.0..AND.TOLT.GT.0..AND.
     3       TOLA.GT.0..AND.B.GT.SQRT(FLOAT(NP)).AND.C.GT.0.
      IF (.NOT.NPRCHK) CALL MESSGE(500,'GITAC2',1)
      IUCO=IUCV
      A2O=A2
      B2O=B2
      IUCV=1
...
C
      DOUBLE PRECISION GG,DMIN,DMAX,XMIN,YMIN,T1,T2,S1,S2,DXLOG,XBIG,YY
...
C
  ...
  100         ENI=ENI+1
              ...
              CALL BIGGBI(T01,DNI,DMIN,DMAX,S2)
              ELG0=SNGL(-YY*T01+ENI*S2)
              CALL BIGGBI(T1,DNI,DMIN,DMAX,S2)
              ELG1=SNGL(-YY*T1+ENI*S2)
              CALL BIGGBI(T2,DNI,DMIN,DMAX,S2)
